...
SECONDS=0
ZIPNAME="Priyadi-vayu-$(date '+%Y%m%d-%H%M').zip"

if ! [ -d "$HOME/tc/proton-clang" ]; then
echo "Proton clang not found! Cloning..."
if ! git clone -q --depth=1 --single-branch https://github.com/aguspriyadi/proton-clang.git ~/tc/proton-clang; then
...
mkdir -p out
make O=out ARCH=arm64 priyadi_vayu_defconfig 
...
git clone -q https://github.com/aguspriyadi/AnyKernel3.git 
